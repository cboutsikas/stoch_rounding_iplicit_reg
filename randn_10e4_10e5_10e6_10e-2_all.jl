...
This code will generate the matrices as described in Section 5.3 (Full rank matrices).
...
function create_store_mats_sigma_min(n::Int, d::AbstractArray, dec_pos::AbstractArray, name_A_G::String, name_A_Gk::String, 
    name_A_Gk_2::String, k::Int)
    l = length(d); sigma_min = 10^(-2); g = length(dec_pos); S_all = -ones(l,1+3*g); V_all = -ones(l, 3*g); V_norm_all = -ones(l, 3*g);
    ...
    for j=1:l
        ...
        writedlm(name*"10e-2_all_sigmas.txt", S_all);

        FileIO.save(name*"10e-2_all_vars.jld2", "V_all", V_all);
        writedlm(name*"10e-2_all_vars.txt",V_all);
